...
class TestRunner:
    ...
    def run_test(self, cmdline, expected_rc=0, expected_stdout=None,
                 expected_stderr=None, test_name=None, stdin=None,
        ...
        which use them have been successful.187

        EXPECTED_STDOUT, EXPECTED_STDERR, and STDIN may be specified as `str`s
        or `byte`s.  But it must be consistent within a call you cannot specify
        ...
        """
        if expected_stdout is None and expected_stderr is None:
        ...
        if rc.returncode != expected_rc:
            ...
            self._fails += 1
            success = False
        elif not self._check_expected_output(rc.stdout, expected_stdout,
                                             test_name, "stdout"):
            success = False
        elif not self._check_expected_output(rc.stderr, expected_stderr,
                                             test_name, "stderr"):
            success = False
        else:
